...
Begin VB.Form FrmHoldChequeEntry 
   BackColor       =   &H00FFC0C0&
   ...
   Begin MSFlexGridLib.MSFlexGrid Grid 
      ...
      AllowUserResizing=   1
      Appearance      =   0
      FormatString    =   "<Cheque Barcode No|<CC Cheque No|<CC Cheque Amount|<Hold Entry Date|<Centre Name|<Cheque Date"
   ...
   End
End
Attribute VB_Name = "FrmHoldChequeEntry"
...
Grid.Rows = 1
rs1.Open "select Dispatchslipcode, CCChqIdNo, CCChequeNo, CCChequeAmt, HoldEntryDate, CentreChequeDate from ASPDC_DispatchSlipDetails ADS where HoldFlag =1 and UnHoldFlag =0 and HoldEntryDate >='" & Format(dtFrom.Value, "dd Mmm yyyy") & "' and HoldEntryDate <='" & Format(dtTo.Value, "dd Mmm yyyy") & "' order by HoldEntryDate, CCCHQIdNo", cn1, adOpenDynamic, adLockReadOnly
If Not (rs1.BOF And rs1.EOF) Then
    ...
    Do While Not rs1.EOF
        ...
        Grid.TextMatrix(Grid.Rows - 1, 3) = Format(rs1.Fields("HoldEntryDate").Value, "dd Mmm yyyy")
        Grid.TextMatrix(Grid.Rows - 1, 4) = Left(rs1.Fields("Dispatchslipcode").Value, 5)
